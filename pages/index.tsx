...
const Home: NextPage = () => {
  ...
  const calculatePoints = (grim:any) => {
    ...
    return [Utils.formatPoints(`${unclaimedPoints}`), Utils.formatPoints(`${lockedPoints}`)];      
  }
  ...
  const calculateAvailablePoints = (grim:any) => {
    ...
    return Utils.formatPoints(`${parseFloat(unclaimed) + parseFloat(locked)}`);
  };
  ...
  const updateAstraCalc = () => {
    ...
    setPoints(ps);
    setPointsUnclaimed(parseFloat(Utils.formatPoints(`${allUnclaimedPoints}`)));
    setPointsLocked(parseFloat(Utils.formatPoints(`${allLockedPoints}`)));
  }
